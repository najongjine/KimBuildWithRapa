...
public class LoginController {
	...
	public String loginProcess(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, ModelMap model,
		...
		if (loginVO.getId() != null && loginVO.getPwd() != null && !"".equals(loginVO.getId())
			...
			if (loginFailedCnt > 5) {
				model.addAttribute("message", "로그인 실패가 5회를 초과하여 IP 가 정지 당하셨습니다.");
			 ...
			 * log01VO.setIpErrYn("Y"); 접속 로그 cmmnService.insertContents(log01VO,
			 * PROGRAM_ID);
			 * 
			 * model.addAttribute("message", "아이피를 확인하시기 바랍니다."); status.setComplete();
			 * return "/ma/login/login"; }
			 */
