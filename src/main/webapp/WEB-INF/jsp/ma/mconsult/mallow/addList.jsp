				...
				<c:when test="${fn:length(resultList) > 0}">
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr class="cursor">
							...
							<td onclick="fncPageBoard('view','view.do','${result.seq}','seq')">
								<c:choose>
									<c:when test="${result.bStatus eq 'N' }">미승인</c:when>
									<c:when test="${result.bStatus eq 'O' }">승인</c:when>
									<c:when test="${result.bStatus eq 'R' }">발료</c:when>
									<c:when test="${result.bStatus eq 'P' }">재승인요청</c:when>
									<c:when test="${result.bStatus eq 'D' }">영구발료</c:when>
								</c:choose>
							</td>
